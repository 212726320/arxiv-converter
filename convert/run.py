# File: run.py
# File Created: Saturday, 31st August 2019 3:22:25 pm
...
def run():
    ...
    # project)
    tex_lines = files.insert_inputs(args.src_dir, main_file)

    # Remove comments
    # TODO also comments that end a line
    tex_lines = files.remove_comments(tex_lines)

    # Flatten figure files, replacing forward slashes with underscores
    # The naming convention is "fig1.pdf", "fig2.eps", etc.
    tex_lines_with_href, dest_fig_files = files.clean_figs(tex_lines, True)
    # args.pdf_figs_only)

    # Checks for \usepackage{hyperref} and comments it out.
    tex_lines = files.comment_hyperref(tex_lines_with_href) \
